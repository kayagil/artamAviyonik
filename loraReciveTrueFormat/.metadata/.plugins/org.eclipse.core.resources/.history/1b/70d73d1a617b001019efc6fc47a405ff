...
void LoRa_SendMessage(const char* message)
{
  ...
  int i = 0;
  while (message[i] != '\0') {
    RFM98_Write(0x00, message[i]); // RegFifo
    i++;
  }

  // 4. Gönderilecek paketin uzunluğunu ayarla.
  RFM98_Write(0x22, i); // RegPayloadLength
...
uint8_t dizi[4] = {1,2,3,4};
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	      ...
	      // 1. Bir sonraki gönderim için bayrağı sıfırla
	      tx_done_flag = 0;

	      // 2. Mesaj gönderme işlemini başlat (bu fonksiyon artık beklemiyor, hemen geri dönecek)
	      LoRa_SendMessage(dizi);

	      // 3. Interrupt'ın bayrağı 1 yapmasını BEKLE
	      //    tx_done_flag, interrupt fonksiyonu içinde 1 yapılır.
	      ...
	      // 4. Gönderim bitti, şimdi bir sonraki gönderim için 5 saniye bekle
	      HAL_Delay(5);
