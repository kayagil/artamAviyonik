...
{
  // 1. Modülün Bekleme (Standby) modunda olduğundan emin ol.
  RFM98_Write(REG_OP_MODE, MODE_STDBY);

  // 2. FIFO (modülün içindeki veri hafızası) yazma başlangıç adresini ayarla.
  RFM98_Write(0x0E, 0x80); // RegFifoTxBaseAddr
  RFM98_Write(0x0D, 0x80); // RegFifoAddrPtr




  // 3. Gönderilecek struct'ı bir byte dizisi olarak ele al ve FIFO'ya yaz.
    //    (TelemetryPacket_t*) tipindeki işaretçiyi (uint8_t*)'a çevirerek
    //    struct'ın bellekteki her bir byte'ına sırayla erişebiliriz.
    uint8_t* data_ptr = (uint8_t*)packet;
    uint8_t packet_size = sizeof(TelemetryPacket_t); // Boyutu otomatik hesapla (60 byte olmalı)


int a;

  // 3. Gönderilecek mesajı byte byte FIFO'ya yaz.
    for (int i = 0; i < packet_size; i++)
    {
      RFM98_Write(0x00, data_ptr[i]); // RegFifo'ya sıradaki byte'ı yaz
      a = i;
    }
  // 4. Gönderilecek paketin uzunluğunu ayarla.
  RFM98_Write(0x22, a); // RegPayloadLength

  // 5. Gönderme (TX) moduna geçerek işlemi başlat.
  RFM98_Write(REG_OP_MODE, MODE_TX);

//  // 6. Gönderme işleminin bitmesini bekle.
//  //    Bunu anlamak için TxDone bayrağını (flag) kontrol ediyoruz.
//  //    (Daha verimli yöntem kesme (interrupt) kullanmaktır, ama başlangıç için bu daha basit)
//  while ((RFM98_Read(0x12) & 0x08) == 0); // RegIrqFlags'daki TxDone bitini kontrol et
//
//  // 7. TxDone bayrağını temizle.
//  RFM98_Write(0x12, 0x08); // RegIrqFlags
}
...
telemetry_data.packet_number = 0;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // 1. Telemetri verilerini struct'a doldur (Bunlar normalde sensörlerden vb. gelecek)
	      telemetry_data.packet_number ++; // Her döngüde paket numarasını artır
	      telemetry_data.flight_state = 4; // Örnek: AFTER_BURNOUT
	      telemetry_data.obc_altitude = 2332;
	      telemetry_data.prc_voltage = 1628; // Voltaj 16.28V ise 100 ile çarpıp int16 olarak saklanabilir.
	      telemetry_data.latitude = 38.402641f;
	      telemetry_data.longitude = 33.706203f;
